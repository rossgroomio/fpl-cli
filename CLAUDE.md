...
API clients in `fpl_cli/api/`: FPLClient (main API, caches `bootstrap-static/`), fpl_draft, understat (scrapes understat.com for npxG/xGChain/xGBuildup), historical data via dual-source architecture: VaastavClient + CoreInsightsClient (both multi-season, the window passed in as hyphenated labels by the caller, keyed on `element_code`; Core-Insights defaults to the current season alone and reads its per-gameweek files from the newest configured season), composed by `HistoricalDataProvider`, which keeps one row per `(element_code, season)` with Core-Insights outranking vaastav. The split is `historical_season_windows()`'s (`api/historical.py`), not either client's: a four-season trailing window, the newest two from Core-Insights (all it publishes), the two before from vaastav — never hardcode a season label there, never widen one side without narrowing the other, and `fpl doctor --providers` probes the same allocation; the window rolls itself at the July cutover. Shared types in `historical_types.py` (SeasonHistory, PlayerProfile, GwTrendProfile). Season helper: `season_label()` (alias `vaastav_season()`). Scraper in `fpl_cli/scraper/` (credentials resolve `FPL_EMAIL` / `FPL_PASSWORD` first, then the system keyring written by `fpl credentials set`; set `FPL_BROWSER_IGNORE_CERTS=1` behind TLS-inspecting proxies — Chromium ignores `SSL_CERT_FILE` / `REQUESTS_CA_BUNDLE`). Jinja2 templates in `templates/`.
...
- Table mode splits the same way: the table on stdout, and every other line — warnings, progress notices, and the reason for a nonzero exit — on stderr, so `2>/dev/null` means the same thing on every command. Report a failure with `emit_failure()` (or `handle_agent_failure()` for an agent's), and where a call site prints its own prose before `raise SystemExit(1)`, print it on `error_console`, never on `console`
- A failure raised too deep to know the command name or the format goes through a boundary: `api_failure_boundary(command, output_format)` around the `asyncio.run()` for an unreachable upstream, and the `@config_failure_boundary` decorator on the command callback for a `settings.yaml` block a parser rejected. A config parser raises `ConfigError` (`cli/_context.py`) and nothing wider, so the boundary catches the defect and a bug in our own code still surfaces as a traceback — a bare `ValueError` escapes click and, under `--format json`, exits 1 with zero bytes on stdout (#170)
...
- **Context access:** A command reads its settings with `get_settings(ctx)` and its format with `get_format(ctx)` — never by unwrapping `ctx.obj` and never by calling `load_settings()` a second time; `main()` already merged them onto the context. A ruff `TID251` rule enforces both, banning the `CLIContext` and `load_settings` imports in `fpl_cli/cli/` outside `__init__.py` and `doctor.py`. `load_settings()` is for callers with no context (agents, services) and for `fpl doctor`, which runs on an empty `CLIContext` so each check re-reads by design. A helper too deep to hold a context takes the value it needs as an argument (see `transfer_eval.py`'s `_render_table`)
- **Custom-analysis gate:** `custom_analysis_enabled(ctx)` when the toggle is all the command needs from the settings (`fdr`, `fixtures`, `xg`); `is_custom_analysis_enabled(settings)` on the dict `get_settings(ctx)` already returned when it reads other keys too (`stats`, `player`, `preview`, `status`) — so the gate answers for the same settings the rest of the command is reading
...
## Rules
- This repo is public: never put real manager or league names, entry/league IDs, or FPL account details into commits (test fixtures included), PR titles or bodies, issues, or CHANGELOG.md. Use placeholders when quoting generated reports from the configured `reports.output_dir`, including in an issue's repro example — copy its shape, not its names. Footballer and club names are fine — they are public data. Before pushing, check the branch for a pre-existing leak and flag it rather than pushing over it.
