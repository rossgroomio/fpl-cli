...
2. **[Early-season prior blend](#early-season-confidence-gw1-10)** of that baseline (GW1-9), against the family's calibrated anchor
3. **Underperformance regression bonus** for players outperforming xG
4. **3-GW matchup** (scalar average, weight 0.75 via `_matchup_bonus`)
5. **Availability penalty** (-3pt when status flagged < 75%) -- **waiver only in practice**, see below
6. All three include `penalty_xG` via `StatWeight`

Step 2 sits where it does deliberately: everything below it is a statement about the player's fixtures, ownership or squad fit, which a previous-season prior says nothing about.
...
Scores normalised to 0-100 and ranked on that value. Before GW10 the baseline carries the [prior blend](#early-season-confidence-gw1-10); there is no separate shrinkage pass afterwards.
...
**Keepers:** the three GK signals come from the main-game player each draft element is joined to on `code` — the draft bootstrap publishes neither saves nor expected goals conceded, so they cannot be read off the draft record itself. A keeper the join misses falls back to form and ppg alone and keeps the unscaled anchor (see [Quality & Value Scores](#quality--value-scores)).
...
### Value and ownership families: prior blend

`quality_score` (`fpl stats --value`, `fpl player`, `fpl transfer-eval`, and `fpl allocate` at horizon >= 2, where the solver ranks on it) and the ownership scores (`fpl targets`, `fpl differentials`, `fpl waivers`, and transfer-eval's Outlook column) blend the observed raw score with the score the prior implies:

```
prior_raw = prior_strength × anchor × 0.92      # the elite target the ceilings were calibrated to
w         = confidence
blended   = w × observed_raw + (1 − w) × prior_raw
```

Going into GW2 the observed score is one gameweek -- form and ppg are the same single observation and both caps saturate on one good game -- so without the prior a one-game wonder out-reads a quiet-starting elite (issue #143: Haaland 59 behind Emersonn 100). Ceilings cannot reorder that, being monotonic per-position scalers, so the prior enters the score itself. `anchor` is the calibrated elite raw score the family measures against for that position at that gameweek (the calendar-attainable one for a pre-GW6 keeper), so a player at the top of last season's percentile is read as exactly the elite the calibration anchored and a price-sourced prior can never claim more than mid-pack -- 92 and 46 on the value scale, which has no bonus terms. The weight is the same for every position. A keeper-specific discount (the GK calendar ramp, on the grounds that a keeper's early signals are sample-ramped) was evaluated and not shipped: the discounted share can only move onto the prior, so the same pedigree and the same empty observation read 75 for a keeper and 43 for a forward going into GW2, and `fpl allocate` sums raw quality across positions; the within-position gain it bought (+0.08 rank correlation on ~24 keepers a snapshot) sat inside the noise. The backtest's finding that the prior alone out-ranks the blend for keepers at several early snapshots stands, and a better keeper prior is the route that leaves the weight alone.

**Where the ownership family differs.** Its scores are the quality baseline plus a 3-GW matchup bonus, an ownership bonus (differentials), a position-need and team-stacking adjustment (waivers) and a consistency bonus. The blend runs on the baseline alone, before any of those are added, and `anchor` is correspondingly the family's ceiling *without* their headroom. A prior is a statement about the player -- last season's pts/90, or price for a player with no PL history -- and models none of the fixtures in front of them, how many managers own them, or which slot of your squad is thin, so blending it into those terms would credit a pedigree for a fixture run it never played. Everything above the baseline stays pure observation.

Backtested before shipping (`scripts/backtest_early_season_prior_blend.py`: 2025-26 GW1-7 snapshots, 2024-25 priors, Spearman rank correlation with rest-of-season points, blended minus the incumbent):

| family | incumbent | GK | DEF | MID | FWD |
|---|---|---|---|---|---|
| value | pure observation | +0.13 | +0.07 | +0.01 | +0.17 |
| target | shrinkage | +0.16 | +0.08 | +0.01 | +0.23 |
| differential | shrinkage | +0.16 | +0.08 | +0.01 | +0.23 |
| waiver | shrinkage | +0.14 | +0.08 | +0.02 | +0.22 |

Better at every snapshot for GK, DEF and FWD; MID is neutral throughout, as it was for the value family. On a six-gameweek rather than a rest-of-season outcome the ownership picture is the same for GK (+0.13) and FWD (+0.16) and mildly negative for DEF (-0.03) and MID (-0.04) -- these are multi-gameweek outlook scores, so rest-of-season is the outcome they are read on, but the shorter horizon is where a strong-pedigree player who is genuinely out of form costs you. The blend is a no-op by GW8 for players with a strong history and by GW10 for everyone. The script scores the blend through the production function and the shrinkage baseline through `shrink_scores`, so re-running it after a change to either, to the confidence curve or to the anchors measures what ships.
...
Shrinkage compresses gaps rather than reordering them: it only swaps two players when their confidences differ enough to overcome the gap between their scores, which is far too weak to move a one-game wonder off the top of a list. That is exactly why the two multi-gameweek families moved to the blend. It survives here because a single-gameweek score is a projection of one match, where a pedigree from a different season is a weaker claim than it is over a whole rest-of-season horizon, and because bench and lineup order on raw scores -- for those two, shrinkage changes only the displayed number.

Both are the player-level analogue of the team-level early-season blending in [Team Ratings](#early-season-blending-gw1-11).

### Who is left out

Both devices treat a low score as a small sample, so they only make sense for players whose score could plausibly be higher. Two groups are held out entirely -- excluded from the position mean as well as from the adjustment by shrinkage, and returned with their observed score by the blend:
...
The general FDR shown beside them is the mean of the two, in the same mode and at the same venue. `TeamRatingsService.get_fixture_fdr()` is its only definition, so a match carries one number on every surface that shows one: everything the fixture agent produces (`fpl fdr` and `fpl preview`, including the per-fixture `home_fdr`/`away_fdr` in `fpl fdr --format json`) and the direct-api `fpl fixtures` table, which kept a second copy of the old model until #202. All three follow the `custom_analysis` toggle together: with it off they show the FPL API's 1-5 difficulty instead, so they agree on a match in either configuration rather than only in one. It replaced the opponent's venue-blind average rating, which ranked the league's weakest side above its strongest whenever the strong side's run held big names (#186). A fixture involving an unrated club scores the neutral 4.0 on all three columns rather than falling back to the FPL API's 1-5 difficulty, which would put that club on a second scale inside the same ranking.

`fpl chips timing` grades a Triple Captain candidate's double on the same general FDR, averaged across that candidate's fixtures in the gameweek. It previously read the candidate's *own* club's `avg_overall_fdr` (the venue-blind opponent average, since removed from `TeamRating`), which is the difficulty of facing that club - the inverse of the intent, so the signal fired for the weakest teams and never for the strongest (#201). It is the one FDR surface that does **not** grade an unrated club at the neutral 4.0: a placeholder clears its "possible" threshold, so an unrated candidate is skipped rather than recommended off a number that carries no fixture read.
...
A club is promoted, for the prior's purposes, when last season's Premier League pool holds no completed record for it - and that pool is guarded on two sides, because Understat's team endpoint never says no. Asked for a season a club has no record of, it serves that club's most recent season instead: for a side just promoted, the season now in progress. A club back after exactly one season away (Ipswich in 2026-27, relegated in 2025 and straight back up) therefore answered a request for 2025-26 with its 2026-27 fixture list, and once GW1 had kicked off the one result in it was read as a season of Premier League xG - 1.18 xGA in a single home match, the away venue estimated from it, bucketed against nineteen full seasons to a def 2/2 prior while the two other promoted clubs read as bottom-of-table (#235). So `UnderstatClient.get_team()` keeps only matches whose kickoff falls in the season it asked for - the dated matches decide, so a lone undated entry cannot keep a substituted squad alive, and a payload with no dates at all is treated as shape drift and kept - and reports a club with none as having no record. Separately, `generate_prior()` holds every source to a full-season bar: a club showing fewer than `PRIOR_MIN_GAMES_PER_VENUE` (10) matches at either venue is not ranked, since a completed season is 19 of each and a fragment in a pool of full seasons is wrong whatever put it there, and the same bar drops a partial club from the Championship division before the promoted cohort is z-scored against it. The damping itself already keeps a promoted side out of the top third - the level term sits below the league on every axis, and the measured spread cannot carry a Championship edge that far - so no separate cap is applied; the bug was a side that never reached the damping at all.

What a club that fails the pool gets depends on why. A club with no Premier League record at all takes the promoted path: its Championship record, else the flat bottom-of-table estimate. A club whose record was served but is not a season's worth is looked up in the Championship too (in case the season guard was inert), and if nothing is found it takes the neutral mid-table rating rather than the promoted estimate, since a club a source has a Premier League page for is far more likely a continuing one whose fetch broke than a promoted one. That ambiguity cannot be resolved from the data - a promoted club can be missing from the Championship record too, as both Sheffield clubs were for the tla collision - so it is made loud instead: the league promotes exactly three clubs (`PROMOTED_CLUBS_PER_SEASON`), and a fourth club absent from the pool is warned about by name as a continuing club that failed to join.

Every rating the prior produces is traceable. `team_ratings_prior.yaml` (data dir) carries an `inputs` block beside the ratings: per club its `basis` (`premier_league`, `championship`, `promoted_fallback`, or `incomplete_record`), the matches behind it, the per-game rates that were bucketed (`ranked`, in Premier League units) and, for a Championship side, the rates as played (`played`) before the conversion - both sides of the damping; an incomplete record shows what was `served`. `metadata.promoted` lists the clubs rated from outside the Premier League pool, `metadata.incomplete` the fragments, and `metadata.based_on_season` names the season the prior describes. The trace is read through the same version check as the ratings, so a file an older methodology wrote never shows a trace for a prior the tool would not serve. `fpl ratings update` prints the same summary wherever it applies the prior: which clubs it rates from Championship results, which sit on the flat estimate or the neutral rating, and where the per-club inputs are.
...
The comparison runs one way by construction. It asks whether an input the cached run *lacked* is available now, never whether the two runs differ, so a transient outage or a key removed leaves a good prior alone instead of replacing it with a worse one.

The rebuild is held to the same standard, because "non-empty" is not the same as "better". A rebuild that succeeds in form but not in substance - Understat failing mid-run so the pool drops to football-data's noisier goals, or football-data rate-limiting so the promoted sides fall back to the flat estimate - produces a full, plausible table that would otherwise overwrite a better one and then latch, since the file it writes records that these inputs have been tried. So a rebuilt prior is compared against the cached one on the two things the cache records about its own quality - which source measured the Premier League pool, and how many clubs ended up on a rating better evidence would have improved - and is refused if either regressed. Refused, empty, or accepted, the attempt itself is recorded on the file, which is what bounds a down provider to one retry per input change rather than one per command. A cache that fails the *club-list* check is never held as a fallback, since it is wrong for this league rather than merely improvable.

That bound is a real trade: an attempt spent against a provider that was briefly down is not retried automatically. The state is visible rather than silent - with a key set and clubs still on the flat estimate, the summary reads `` COV, HUL, IPS on the flat promoted estimate (football-data.org served no Championship record for them; `fpl ratings update --refresh-prior` retries) `` - so the way back is named in the output rather than being a file to delete.

`fpl ratings update --refresh-prior` forces a rebuild for a reason provenance cannot infer - a source that answered, but answered wrongly. It opens the cache gate, not the quality guard, so it cannot downgrade a prior either. It is the escape hatch, not the remedy: nothing about a changed input should need it.
...
**Early-season prior (GW1-10).** Per-player confidence is derived from prior-season pts/90 (from the historical datasets) via `player_prior.py`, and `prepare_scoring_data(include_prior=True)` fetches priors into `ScoringData.player_priors`. The value and ownership families blend the prior into the quality baseline inside the score (`blend_quality_with_prior()`), so the agent passes the player's `PlayerPrior` to the scoring call and ranks on what comes back. The single-GW family still shrinks normalised scores toward the position mean via `shrink_scores()`, called through `apply_shrinkage()` between scoring and ranking with the hold-out set `unavailable_player_ids()` builds from live player data. See [Who is left out](#who-is-left-out).
