...
`fpl chips timing` grades a Triple Captain candidate's double on the same general FDR, averaged across that candidate's fixtures in the gameweek. It previously read the candidate's *own* club's `avg_overall_fdr`, which is the difficulty of facing that club - the inverse of the intent, so the signal fired for the weakest teams and never for the strongest (#201).
