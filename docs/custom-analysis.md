...
The calibrated ceilings live in `QUALITY_CEILINGS` in `services/scoring/constants.py`, in a generated block the script rewrites with `--write`, alongside a fingerprint of every scoring input the calibration depends on (weights, position multipliers, signal clamp bounds). A drift-guard test recomputes that fingerprint, so changing any weight without re-running the calibration fails CI rather than silently mis-scaling scores — the historical failure mode where theoretical cap sums assumed signal saturation only DEF actually exhibits, leaving elite MID/FWD/GK reading ~60 where elite DEFs read ~89 (issue #88).
