...
A club is promoted, for the prior's purposes, when last season's Premier League pool holds no completed record for it - and that pool is guarded on two sides, because Understat's team endpoint never says no. Asked for a season a club has no record of, it serves that club's most recent season instead: for a side just promoted, the season now in progress. A club back after exactly one season away (Ipswich in 2026-27, relegated in 2025 and straight back up) therefore answered a request for 2025-26 with its 2026-27 fixture list, and once GW1 had kicked off the one result in it was read as a season of Premier League xG - 1.18 xGA in a single home match, the away venue estimated from it, bucketed against nineteen full seasons to a def 2/2 prior while the two other promoted clubs read as bottom-of-table (#235). So `UnderstatClient.get_team()` keeps only matches whose kickoff falls in the season it asked for and reports a club with none as having no record, and `_prior_from_understat()` additionally leaves out any club showing fewer than `PRIOR_MIN_GAMES_PER_VENUE` (10) matches at either venue, since a completed season is 19 of each and a fragment in a pool of full seasons is wrong whatever put it there. Either guard sends the club down the promoted path. The damping itself already keeps a promoted side out of the top third - the level term sits below the league on every axis, and the measured spread cannot carry a Championship edge that far - so no separate cap is applied; the bug was a side that never reached the damping at all.

Every rating the prior produces is traceable. `team_ratings_prior.yaml` (data dir) carries an `inputs` block beside the ratings: per club its `basis` (`premier_league`, `championship`, or `promoted_fallback`), the matches behind it, the per-game rates that were bucketed (`ranked`, in Premier League units) and, for a Championship side, the rates as played (`played`) before the conversion - both sides of the damping. `metadata.promoted` lists the clubs rated from outside the Premier League pool and `metadata.based_on_season` names the season the prior describes. `fpl ratings update` prints the same summary wherever it applies the prior: which clubs it rates from Championship results, which sit on the flat estimate, and where the per-club inputs are.
