...
**GK ceilings scale with sample size:** all three GK signals (saves/90, xGC quality, CS rate) are multiplied by a deliberate small-sample ramp, `min(minutes / 450, 1)` — but the calibrated anchors were measured at full ramp, which capped the entire GK pool in the low 70s at GW2 no matter how anyone played. `gk_ceiling_attainability` therefore scales the anchor's GK-signal share (from the weight caps) by the same ramp, so an early-season keeper is read against what a keeper *could* have shown at that sample size. From 450 minutes (~GW5 for an ever-present) the scaling is the identity and full-sample scores are untouched.
