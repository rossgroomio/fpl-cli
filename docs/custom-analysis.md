...
- **GK**: dedicated signals via `for_gk()` weights — saves per 90, defensive quality (inverted xGC/90, range 0-2), and clean sheet rate. Raw quality attenuated by 0.7 and normalised against `GK_VALUE_CEILING` (~19.7), derived from the `for_gk()` cap sum × 0.7.
- **DEF**: `dc_per_90` (defensive contribution rate) replaces attacking xG stats via `without_xgi()`. Raw quality attenuated by 0.85 and normalised against `DEF_VALUE_CEILING` (~14.2), derived from the `without_xgi()` cap sum × 0.85 — **not** the MID/FWD-anchored `VALUE_CEILING`. This replaces a prior MID-anchored scaling that produced a mathematical no-op (both numerator and denominator scaled by 0.85) and compressed real DEF pools into a narrow upper band on ownership scores.
- **MID/FWD**: unchanged — multiplier 1.0, ceiling `VALUE_CEILING` (24.3).

The same four families (target, differential, waiver, value) use analogous per-position ceilings: `GK_TARGET_CEILING` / `DEF_TARGET_CEILING`, etc. A DEF pool spanning scrub→elite now shows a spread of ~30 points across the 0-100 range on each family, up from ~5 points pre-fix.
