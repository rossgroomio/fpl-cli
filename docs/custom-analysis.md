...
`PL_POOL_RELIABILITY` is the same quantity for the pool being ranked in: a true talent gap is a smaller share of a noisy pool's observed spread than of its true one. Measured at 0.57 over the 2025-26 Understat xG pool, taking each team's sampling variance directly from its match-to-match xG rather than assuming Poisson, which does not hold for xG.
