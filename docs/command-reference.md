...
A waiver claim a rival won is reported under **Claims Lost** rather than discarded, and reaches the personal analysis alongside the moves that landed, with the priority it was made at. A gameweek whose only waiver activity was a claim you lost is not a gameweek without waiver activity, and "No waivers this week" is now said only when nothing was submitted either. A recommendation you acted on and were beaten to reads as "claimed, lost to a rival" in the recommendation comparison rather than "not executed". A claim denied because an earlier accepted claim of yours had already dropped the nominated player is a conditional-chain cascade, not an attempt, and is not reported as one.
