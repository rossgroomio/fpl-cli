...
**When one player fails to join.** A player whose club Understat *does* carry can still fail to match on name, and that is not always a defect: Understat has not ingested the gameweek in progress, spells a transliterated name differently, or simply has no row for the player. Those read as `null` quality and value scores rather than as zeros, so the player is ranked as "no data" rather than as bad. A few percent of a club's players missing this way is normal and stays a debug line naming each one with a running rate (`No Understat row matched ... — 3 of 351 FPL players with minutes unmatched so far this run`); the rate is what to watch, since a jump in it means the two sources have stopped agreeing about names rather than that one more cameo went un-ingested. `fpl doctor --providers` reports the same rate as an `Understat name join` row, which is where to look for it without turning on debug logging. Understat serves apostrophes HTML-escaped (`O&#039;Shea`), which used to fail every apostrophe name silently; the payload is now decoded when it is fetched.
