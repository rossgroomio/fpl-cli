...
│   ├── review.py                 # Review research + synthesis prompts. Squad lines in the synthesis data block carry each player's full club name, and the hard constraints bind club affiliation and blanket scored/blanked claims to that supplied data — an LLM's own club knowledge goes stale every transfer window
...
│   └── league_recap.py           # League recap synthesis prompts, including the anchored League History section, its never-infer-history rule (emitted even when the pack is empty), and the season-phase framing instruction. `format_recap_player_clubs_context()` builds a `## Player Clubs` roster (every player in the gameweek's squads and transfers, resolved to a full club name) and the captain groups carry their club inline, so club claims bind to supplied data instead of recall; a name the data gives two clubs for is dropped rather than guessed at
├── parsers/
