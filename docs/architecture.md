...
└── utils/
    ...
    ├── text.py                   # strip_diacritics (name matching across sources). normalise_name folds the rest of what a retyped name varies by — the apostrophe family onto U+0027, whitespace runs (NFKC, so non-breaking spaces too) and the space after an initial ("B. Fernandes" → "B.Fernandes") — on top of the diacritic strip and lowercasing. Apply it to both sides of a comparison: a `\b`-anchored regex survives none of those differences, and each one silently cost a valid row in the review's Disappointments table (#343). Also ordinal_suffix/ordinal_word
