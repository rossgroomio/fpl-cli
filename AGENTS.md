...
- **Context access:** A command reads its settings with `get_settings(ctx)` and its format with `get_format(ctx)` — never by unwrapping `ctx.obj` (a ruff `TID251` rule bans importing `CLIContext` for it) and never by calling `load_settings()` a second time; `main()` already merged them onto the context. `load_settings()` is for callers with no context (agents, services) and for `fpl doctor`, which re-reads per check by design
