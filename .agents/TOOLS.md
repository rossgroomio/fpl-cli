...
- `_md_sections.py` (`.agents/skills/gw-prep/scripts/`) — shared helper both validators use to locate a markdown section by heading. Tolerates sub-agent heading drift (qualifiers, case, leading annotations, opt-in aliases) without matching a different heading that shares a prefix.
