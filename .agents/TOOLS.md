...
| `squad sell-prices` | Show squad sell prices and financial breakdown. Nothing cached and no `--refresh`, a failed scrape, or draft format each exit 1 with an `error` envelope; a scrape the extraction heuristics distrust still returns its data with a `scrape_suspect` entry in `metadata.warnings` | Yes | Classic | No | direct-api |
