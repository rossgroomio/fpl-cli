...
- gw-prep Phase B8 and squad-builder Phase B derive the `fpl stats --min-minutes` floor from the current gameweek (`min(cap, (N - 1) * 45)`, caps 315/450) rather than hardcoding it. A fixed floor is unreachable until GW8/GW11 and empties the result silently -- `fpl stats` returns a well-formed envelope with `"data": []`. gw-prep additionally ranks on `ep_next` before GW6, labels each block with the sort and floor it used, and annotates an empty one. Enforced by `tests/test_skill_stats_thresholds.py`
