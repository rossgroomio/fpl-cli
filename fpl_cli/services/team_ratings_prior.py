...
# A previous-season prior reads a *completed* Premier League season: 19 home
# and 19 away matches per club. A club showing a fraction of that in the pool is
# not carrying a season's evidence, whatever put it there -- a season Understat
# substituted (#235), a broken fetch, a payload cut short -- and ranking it
# among nineteen full seasons is how one match became a top-tier rating. Half a
# venue's worth is the bar: a real record never comes close to it, so a genuine
# club is never dropped for a match or two Understat is missing, while a
# fragment of the shape that bit is nowhere near it. Below the bar the club is
# left out of the pool, which sends it down the promoted path (Championship
# record, else the flat estimate) -- the same treatment `_matches_to_performances`
# gives a club with a venue missing on the football-data side.
...
# (premier_league / championship / promoted_fallback), the matches behind it,
# the per-game rates that were bucketed (`ranked`, in Premier League units)
# and, for a Championship side, the rates as played (`played`) before the
# conversion. Never used in a calculation -- it is the trace, not the rating.
"""
...
def _prior_inputs(
    ...
    for team, ranked in performances.items():
        entry: dict[str, Any] = {
            "basis": PRIOR_BASIS_CHAMPIONSHIP if team in promoted else PRIOR_BASIS_PREMIER_LEAGUE,
            "home_games": ranked.home_games,
            "away_games": ranked.away_games,
        }
        if team in promoted and championship is not None:
    ...
    data: dict[str, Any] = {
        "metadata": {
            ...
            "promoted": sorted(
                ...
                if entry["basis"] != PRIOR_BASIS_PREMIER_LEAGUE
            ),
        },
        "ratings": {},
...
def load_prior_inputs() -> dict[str, dict[str, Any]] | None:
    ...
    None when there is no cache, or one written before `inputs` existed. Read
    fresh from disk each time: `generate_prior` always leaves the file it
    served from in place, so a read straight after it describes that prior.
    """
    path = prior_config_path()
    if not path.exists():
        return None
...
async def generate_prior(client: FPLClient) -> dict[str, TeamRating]:
    ...
    fallback_teams = promoted - set(ranked_promoted)
    ...
    _save_prior_cache(
        ...
        based_on_season=season_label(prev_season_int),
        inputs=_prior_inputs(performances, promoted, championship, fallback_teams),
    )
    return prior
...
async def _prior_from_understat(
    ...
    try:
        ...
        performances = _full_season_records(performances, season_label(int(prev_season)))
...
def _full_season_records(
    performances: dict[str, TeamPerformance], season: str
) -> dict[str, TeamPerformance]:
    """Drop any club whose record is not a season's worth on both venues.

    See PRIOR_MIN_GAMES_PER_VENUE. A dropped club is named, with the record
    it showed, because the fragment is the finding: it means the source
    served something other than that club's completed season.
    """
    kept: dict[str, TeamPerformance] = {}
    for team, performance in performances.items():
        if (
            performance.home_games < PRIOR_MIN_GAMES_PER_VENUE
            or performance.away_games < PRIOR_MIN_GAMES_PER_VENUE
        ):
            logger.warning(
                "%s shows %dH/%dA matches in the %s Premier League record, not a season's "
                "worth - leaving it out of the prior pool rather than ranking a fragment "
                "against full seasons (it takes the promoted path instead)",
                team,
...
class ChampionshipRecords:
    ...
    Both are keyed by FPL short name over the same clubs.
    """
